...
def plot_pulse_comparison(full_result, first9_result, plot_dir):
    ...
    try:
        ...
        # Save plot
        filename = f"{full_result['Experiment_ID']}Pulse{full_result['Pulse_Number']:02d}_{full_result['Pulse_Type']}_Comparison.png"
...
def plot_individual_pulse(result, plot_dir):
    ...
    try:
        ...
        # Save plot
        filename = f"{result['Experiment_ID']}Pulse{result['Pulse_Number']:02d}{result['Pulse_Type']}{result['Fit_Type']}.png"
        filepath = plot_dir / filename
        plt.savefig(filepath, dpi=300, bbox_inches='tight')
        plt.close()
...
OUTPUT_FILE = '/Users/danielsinausia/Downloads/Cation Sizes/Data Analysis/Exp Fits Charging Discharging/pulse_exponential_fits_results.xlsx'

# Output plots directory
PLOTS_DIR = '/Users/danielsinausia/Downloads/Cation Sizes/Data Analysis/Exp Fits Charging Discharging/Fit Plots'
...
def process_files_with_config():
    ...
    def create_summary_stats(data_df, fit_type_name):
        ...
        # Add b_corrected column if not already present
        if 'Parameter_b_Corrected' not in data_df.columns:
            data_df['Parameter_b_Corrected'] = data_df.apply(
                lambda row: -row['Parameter_b'] if row.get('Data_Inverted', False) else row['Parameter_b'],
                axis=1
            )
        ...
        summary_stats.append({
            ...
            'Avg_Parameter_b_Corrected': successful_fits['Parameter_b_Corrected'].mean() if len(successful_fits) > 0 else np.nan,
            'Std_Parameter_b_Corrected': successful_fits['Parameter_b_Corrected'].std() if len(successful_fits) > 0 else np.nan,
        })
        ...
        for pulse_type in ['Odd', 'Even']:
            ...
            summary_stats.append({
                ...
                'Avg_Parameter_b_Corrected': type_data['Parameter_b_Corrected'].mean() if len(type_data) > 0 else np.nan,
                'Std_Parameter_b_Corrected': type_data['Parameter_b_Corrected'].std() if len(type_data) > 0 else np.nan,
            })
        ...
        for experiment_id in data_df['Experiment_ID'].unique():
            ...
            summary_stats.append({
                ...
                'Avg_Parameter_b_Corrected': experiment_data['Parameter_b_Corrected'].mean() if len(experiment_data) > 0 else np.nan,
                'Std_Parameter_b_Corrected': experiment_data['Parameter_b_Corrected'].std() if len(experiment_data) > 0 else np.nan,
            })
